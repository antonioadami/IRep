...
export default class ImovelRepository implements IImovelRepository {
    public async create(data: ICreateImovelDTO): Promise<IImovelModel> {
        const result = await session.run(
            'MATCH (p:Pessoa {email: $userEmail}) CREATE (p)<-[:PERTENCE]-(i: Imovel{nome: $nome, quartos: $quartos, banheiros: $banheiros, estacionamento: $estacionamento, uuid: $uuid, gas: $gas, internet: $internet}) RETURN i',
            data,
        );
